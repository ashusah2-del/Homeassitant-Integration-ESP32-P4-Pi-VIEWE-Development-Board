...
void GSL3680::update_touches() {
    ...
    for (int i = 0; i < cinfo.finger_num && i < 2; i++) {
        // x_raw_max_/y_raw_max_ are set in setup() from the *native* display
        // dimensions, already swapped when swap_x_y_ is set (see setup()).
        // add_raw_touch_position_() below also swaps cinfo.x/y before bounds
        // checking against those same members, so this check must apply the
        // identical swap first or a swapped 90/270 rotation rejects every
        // real touch outside a shrunken corner of the panel.
        int16_t bx = cinfo.x[i];
        int16_t by = cinfo.y[i];
        if (this->swap_x_y_) {
            std::swap(bx, by);
        }
        const bool touch_in_bounds =
            bx >= this->x_raw_min_ && bx <= this->x_raw_max_ &&
            by >= this->y_raw_min_ && by <= this->y_raw_max_;
        if (touch_in_bounds) {
            selected_touch = i;
            break;
        }
